 ...
 * ********************************************************************
 */
package pt.citi.cs.crdt.benchmarks.TPCW_Riak.populator;
...
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.database.TPCW_Riak_Executor;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.Address;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.Author;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.CCXactItem;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.Country;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.Customer;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.Item;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.Order;
import pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.OrderLine;
import java.util.*;
...
public class RiakPopulator extends AbstractBenchmarkPopulator {
	...
	class ItemPopulator implements Runnable {
		...
		public void insertItems(int n) {
			...
			for (int i = 0; i < n; i++) {
				...
				Item item = new Item(base + i, I_TITLE, pubDate.toString(),
						I_PUBLISHER, I_DESC, I_SUBJECT, thumbnail, image,
						I_COST, I_STOCK, isbn, srp, I_RELATED[0], I_RELATED[1],
						I_RELATED[2], I_RELATED[3], I_RELATED[4], I_PAGE,
						avail.toString(), I_BACKING, dimensions, author);
	...
	class Order_and_XACTSPopulator implements Runnable {
		...
		public void databaseInsert(Order order, List<OrderLine> orderLines,
				pt.citi.cs.crdt.benchmarks.TPCW_Riak.entities.CCXactItem ccXact)
				throws Exception {
			((TPCW_Riak_Executor) client)
					.insertOrder(order, orderLines, ccXact);
		...
		public void insertOrder_and_CC_XACTS(int number_keys) {
			...
			for (int z = 0; z < number_keys; z++) {
				...
				Order order = new Order(order_id, O_C_ID, O_DATE.toString(),
						O_SUB_TOTAL, O_TAX, O_TOTAL, O_SHIP_TYPE,
						O_SHIP_DATE.toString(), O_STATUS, billAddress,
						O_SHIP_ADDR);
				...
				CCXactItem ccXact = new CCXactItem(CX_TYPE, CX_NUM, CX_NAME,
						CX_EXPIRY.toString(), O_TOTAL, O_SHIP_DATE.toString(),
						order_id, country_id);
